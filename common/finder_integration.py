...
if os_vers > 10:
    ...
    from LaunchServices import kLSSharedFileListItemBeforeFirst, LSSharedFileListCreate, LSSharedFileListCopySnapshot, \
        LSSharedFileListItemCopyDisplayName, LSSharedFileListItemResolve, LSSharedFileListItemMove, \
        LSSharedFileListItemRemove, LSSharedFileListRemoveAllItems, LSSharedFileListInsertItemURL
...
NetFS_bundle = initFrameworkWrapper('NetFS', frameworkIdentifier=None,
...
class FinderSidebar(object):
    ...
    def get(self, name):
        item = self.favorites.get('Pvtbox', None)
        if item is None:
            return None
        return item.path()
